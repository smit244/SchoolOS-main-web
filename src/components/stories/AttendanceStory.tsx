...
export const AttendanceStory: React.FC = () => {
  ...
  // Scroll Progress mappings (0 to 1 over 300vh) - DESKTOP ONLY
  ...
  return (
    <section ref={containerRef} className="relative w-full lg:h-[300vh] bg-transparent">
      
      {/* ========================================================= */}
      {/* DESKTOP SCROLL-LINKED STORY (Hidden on Mobile) */}
      {/* ========================================================= */}
        ...
        <div className="relative w-full max-w-6xl h-[600px] flex items-center justify-center">
          ...
          <div className="relative w-[400px] h-[400px] z-10 flex items-center justify-center">
            <svg className="absolute inset-0 w-full h-full overflow-visible" viewBox="0 0 400 400">
              <defs>
                <linearGradient id="lineGlowDesk" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <path 
                ...
                fill="none" stroke="rgba(6,182,212,0.15)" strokeWidth="2" strokeDasharray="4 8"
                className="animate-[spin_60s_linear_infinite] origin-center"
              />
              {faceNodes.map((node, i) => (
                faceNodes.slice(i + 1, i + 3).map((target, j) => (
                  <line key={`line-${i}-${j}`} x1={node.cx} y1={node.cy} x2={target.cx} y2={target.cy} stroke="rgba(59,130,246,0.2)" strokeWidth="1" />
                ))
              ))}
              {faceNodes.map((node, i) => (
                <circle key={`node-${i}`} cx={node.cx} cy={node.cy} r="4" fill="#06b6d4" className="animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
              ))}
              <path id="path-left" d="M 100 200 C -50 200 -50 100 -200 100" fill="none" stroke="url(#lineGlowDesk)" strokeWidth="2" strokeDasharray="4 4" />
              <path id="path-right" d="M 300 200 C 450 200 450 300 600 300" fill="none" stroke="url(#lineGlowDesk)" strokeWidth="2" strokeDasharray="4 4" />
              <motion.circle r="6" fill="#06B6D4" style={{ offsetPath: `path('M 100 200 C -50 200 -50 100 -200 100')`, offsetDistance: particlePathProgress, opacity: particleOpacity }} className="will-change-transform shadow-[0_0_15px_rgba(6,182,212,0.8)]" />
              <motion.circle r="6" fill="#8B5CF6" style={{ offsetPath: `path('M 300 200 C 450 200 450 300 600 300')`, offsetDistance: particlePathProgress, opacity: particleOpacity }} className="will-change-transform shadow-[0_0_15px_rgba(139,92,246,0.8)]" />
            </svg>
            <motion.div style={{ y: laserY, opacity: laserOpacity }} className="absolute top-0 left-[-50px] w-[500px] h-[2px] bg-vision-cyan shadow-[0_0_20px_2px_rgba(6,182,212,0.8)] z-20 flex justify-center will-change-transform">
          ...
          <motion.div style={{ y: dashboardY, opacity: dashboardOpacity }} className="absolute left-20 top-1/4 w-80 bg-white/90 backdrop-blur-xl border border-white rounded-[2rem] shadow-[0_30px_80px_rgba(0,0,0,0.1)] p-6 z-20 will-change-transform">
          ...
          <motion.div style={{ x: notifX, opacity: notifOpacity }} className="absolute right-20 top-1/2 w-96 bg-slate-900/95 backdrop-blur-xl border border-slate-800 rounded-[2rem] shadow-[0_40px_100px_rgba(0,0,0,0.3)] p-6 z-20 will-change-transform">
      ...
      {/* ========================================================= */}
      {/* MOBILE IN-VIEW STORY (Hidden on Desktop) */}
      {/* ========================================================= */}
      <div className="lg:hidden w-full py-16 px-4 flex flex-col items-center overflow-hidden">
        ...
        <div className="text-center w-full mb-12">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 border border-slate-200/50 shadow-sm backdrop-blur-xl mb-4"
          >
            <ShieldCheck className="w-4 h-4 text-vision-blueGlow" />
            <span className="text-xs font-bold text-slate-800 tracking-wider uppercase">Biometric Attendance</span>
          </motion.div>
          <motion.h2 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            className="text-4xl font-display font-bold text-slate-900 tracking-tighter"
          >
            Zero-Touch Attendance.
          </motion.h2>
        </div>
        ...
        >
          <svg className="absolute inset-0 w-full h-full overflow-visible" viewBox="0 0 400 400">
            ...
            {faceNodes.map((node, i) => (
              faceNodes.slice(i + 1, i + 3).map((target, j) => (
                <line key={`mob-line-${i}-${j}`} x1={node.cx} y1={node.cy} x2={target.cx} y2={target.cy} stroke="rgba(59,130,246,0.2)" strokeWidth="1" />
              ))
            ))}
            {faceNodes.map((node, i) => (
              <circle key={`mob-node-${i}`} cx={node.cx} cy={node.cy} r="4" fill="#06b6d4" className="animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
            ))}
          </svg>
          <motion.div 
            animate={{ y: [0, 300, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
            className="absolute top-0 left-[-20px] w-[340px] h-[2px] bg-vision-cyan shadow-[0_0_20px_2px_rgba(6,182,212,0.8)] z-20"
          >
            <div className="absolute bottom-full w-full h-[60px] bg-gradient-to-t from-vision-cyan/30 to-transparent" />
          </motion.div>
          <Fingerprint className="absolute inset-0 w-full h-full text-vision-cyan/5 p-8 animate-pulse pointer-events-none" strokeWidth={1} />
        </motion.div>
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-50px" }}
          className="w-full max-w-[320px] bg-white/90 backdrop-blur-xl border border-white rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-6 mb-8 relative z-20"
        >
          <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
            <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
              <CheckCircle2 className="w-6 h-6 text-emerald-500" />
        ...
        <motion.div 
          ...
          viewport={{ once: true, margin: "-50px" }}
          className="w-full max-w-[320px] bg-slate-900/95 backdrop-blur-xl border border-slate-800 rounded-[2rem] shadow-[0_30px_80px_rgba(0,0,0,0.25)] p-6 relative z-20"
        >
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-vision-blueGlow to-vision-cyan flex items-center justify-center mb-4 shadow-[0_10px_20px_rgba(6,182,212,0.3)]">
            <Bell className="w-6 h-6 text-white" />
          </div>
