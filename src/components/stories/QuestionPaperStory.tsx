    ...
    <section ref={containerRef} className="relative w-full lg:h-[300vh] bg-transparent">
      ...
      <div className="hidden lg:flex sticky top-0 w-full h-screen overflow-hidden items-center justify-center">
        ...
        <div className="relative w-full max-w-6xl h-[600px] flex items-center justify-center">
          ...
          <div className="absolute left-32 top-1/2 -translate-y-1/2 w-[400px] h-[400px]">
             ...
             <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 400">
               <defs>
                 <linearGradient id="vortexGradDesk" x1="0%" y1="0%" x2="100%" y2="100%">
                   <stop offset="0%" stopColor="rgba(139,92,246,0.8)" />
                   <stop offset="100%" stopColor="rgba(236,72,153,0.2)" />
                 </linearGradient>
               </defs>
               <motion.g style={{ rotate: particleRotate, originX: "200px", originY: "200px" }} className="will-change-transform">
                 {[...Array(8)].map((_, i) => (
                   <path 
                     key={i}
                     d={`M 200 200 Q ${200 + Math.cos(i*45) * 300} ${200 + Math.sin(i*45) * 100} ${200 + Math.cos(i*45) * 400} ${200 + Math.sin(i*45) * 400}`}
                     fill="none" stroke="url(#vortexGradDesk)" strokeWidth={2} className="opacity-40"
                   />
                 ))}
               </motion.g>
          ...
          <motion.div 
            style={{ opacity: blueprintOpacity, scale: blueprintScale, x: blueprintX, y: blueprintY }}
          ...
          <div className="absolute right-40 top-1/2 -translate-y-1/2 w-72 h-96 z-20">
             <motion.div 
             ...
             >
                ...
                <motion.div style={{ opacity: stampFlash }} className="absolute inset-0 bg-vision-purple/20 backdrop-blur-sm flex items-center justify-center">
      ...
      <div className="lg:hidden w-full py-16 px-4 flex flex-col items-center overflow-hidden">
        ...
        <div className="text-center w-full mb-12">
          ...
          >
            <Wand2 className="w-4 h-4 text-vision-purple" />
            <span className="text-xs font-bold text-slate-800 tracking-wider uppercase">Bcrypt Secure Paper</span>
          </motion.div>
          <motion.h2 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <motion.div 
          ...
          className="relative w-[300px] h-[300px] flex items-center justify-center mb-16"
        >
           <motion.div 
             animate={{ opacity: [0.5, 1, 0.5] }}
             transition={{ duration: 2, repeat: Infinity }} 
             className="absolute inset-0 bg-vision-purple/30 blur-[40px] rounded-full" 
           />
           
           <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 400">
             <defs>
               <linearGradient id="vortexGradMob" x1="0%" y1="0%" x2="100%" y2="100%">
                 <stop offset="0%" stopColor="rgba(139,92,246,0.8)" />
                 <stop offset="100%" stopColor="rgba(236,72,153,0.2)" />
               </linearGradient>
             </defs>
             ...
             >
               {[...Array(8)].map((_, i) => (
                 <path 
                   key={`mob-vortex-${i}`}
                   d={`M 200 200 Q ${200 + Math.cos(i*45) * 300} ${200 + Math.sin(i*45) * 100} ${200 + Math.cos(i*45) * 400} ${200 + Math.sin(i*45) * 400}`}
                   fill="none" stroke="url(#vortexGradMob)" strokeWidth={2} className="opacity-40"
                 />
               ))}
             </motion.g>
             ...
             <motion.g 
               ...
               style={{ originX: "200px", originY: "200px" }}
             >
               {['?', 'A', 'B', 'C', 'x', '=', '+', 'Q', '1', '2', '?', 'A', 'B', 'C', 'x', '=', '+'].map((char, i) => (
                 <text 
                   key={`mob-char-${i}`}
                   x={200 + Math.cos(i * 21) * (80 + Math.random() * 120)}
                   y={200 + Math.sin(i * 21) * (80 + Math.random() * 120)}
                   fill={i % 2 === 0 ? "#8B5CF6" : "#EC4899"}
                   fontSize={Math.random() * 14 + 10}
                   fontWeight="bold"
                   className="animate-pulse drop-shadow-md"
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-50px" }}
          className="w-full max-w-[280px] h-96 z-20"
        >
           <div className="w-full h-full bg-white rounded-xl shadow-[0_30px_60px_rgba(0,0,0,0.15)] border border-slate-100 p-6 flex flex-col gap-4 relative overflow-hidden">
              <Lock className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 text-slate-50 rotate-[-15deg] pointer-events-none" />
